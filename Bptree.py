...
class Bplus_Tree(object):
    ...
    def search(self, key):
        ...
        if(output == None):
            ...
            key = self.array[output][0]
            value = self.array[output][1]
            print("key|value")
    ...
    def leaf_insert(self, key, value, node):
        if(node.node_set == []):
            ...
            node.node_set.append((key, self.length()-1))
        else:
            for i in range(0, len(node.node_set)):
                if(node.node_set[i][0] >= key): break
            if(node.node_set[i][0] > key):
                self.array.append((key, value))
                node.node_set.insert(i, (key,self.length()-1))
            elif(node.node_set[i][0] < key):
                self.array.append((key, value))
                node.node_set.insert(i+1, (key,self.length()-1))
            else:
                index = node.node_set[i][1]
    ...
    def table_insert(self, key, value):
        node = self.root
        self._insert(key, value, node)
        # print("key: "+ str(key)+", value: "+str(value)+" is inserted/upserted successfully.")
    ...
    def vis_leaf_node(self, node):
        for k_v in node.node_set:
            print(str(self.array[k_v[1]][0]) + "|" + str(self.array[k_v[1]][1]))
    ...
    def load_table(self, table_path):
        ...
        while(True):
            ...
            if(line == ""):
                break
            info = line.split('|')
            self.table_insert(int(info[0]), int(info[1]))
...
if __name__ == "__main__":
    ticks = time.time()
    T = Bplus_Tree(23)    #  The k of the tree can be changed.
